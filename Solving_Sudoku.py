...
class Solving_sudoku:
    ...
    def number_first_in_colum_for_line(self, colum_index: int):
        ''' тестовая функция '''
        count_colum = self.count_ava_num_in_colum(colum_index)
        number_and_cord = {}
        for ind in range(9):
            if self.empty_sudoku[ind][colum_index] == 0:
                ava_num_in_point: set = self.available_numbers(ind, colum_index)
                        ...
                        for index_row in range(start_row, end_row):
                            for index_colum in range(start_colum, end_colum):
                                if self.empty_sudoku[index_row][index_colum] == 0:
                                    ...
                                    ava_num_in_point: set = self.available_numbers(index_row, index_colum)
                                    ...
                                    if step_level_3 == 0:
                                        ...
                                        for number in ava_num_in_point:
                                            if Count_squares[number] == 1 and number not in double_bets_in_squares:
